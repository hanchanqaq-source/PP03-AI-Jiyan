...
async function request<T>(path: string, method: "GET" | "POST" | "PUT" | "DELETE" = "GET", body?: unknown): Promise<T> {
  ...
  if (method !== "GET" && path.startsWith("/radar/sources")) headers["X-PP03-Write-Intent"] = "1";
  if (body !== undefined) {
...
export interface RadarSourceHealth {
  status: "ok" | "failed" | "unknown"; checked_at: string | null;
  http_status?: number | null; error_type?: string | null; error_message?: string | null; last_success_at?: string | null;
}
export interface RadarSource {
  id: string; source_type: RadarSourceType; built_in: boolean; enabled: boolean;
  name: string; display_url: string; hint: string; region: string;
  api_adapter?: string | null; created_at?: string; health: RadarSourceHealth;
}
export interface RadarSourceDefinition {
  source_type: RadarSourceType; name: string; url: string; hint: string; region?: string | null; api_adapter?: string;
}
export interface RadarSourceProbe {
  ok: boolean; source_id?: string; http_status?: number | null; feed_format?: "rss" | "atom";
  item_count?: number; sample_title?: string; sample_link?: string; error_type?: string; error_message?: string; checked_at?: string;
}
export interface RadarSourceListing {
  store_status: "missing" | "ok" | "corrupt"; store_error: string | null; api_adapters: string[];
  industries: { key: string; name: string; accent: string }[];
  summary: { total: number; built_in: number; custom: number; enabled: number }; sources: RadarSource[];
}
...
export const api = {
  ...
  radarSources: () => get<RadarSourceListing>("/radar/sources"),
  testRadarSourceDefinition: (definition: RadarSourceDefinition) => request<RadarSourceProbe>("/radar/sources/test", "POST", definition),
  addRadarSource: (definition: RadarSourceDefinition) => request<RadarSource>("/radar/sources", "POST", definition),
  testRadarSource: (sourceId: string) => request<RadarSourceProbe>(`/radar/sources/${encodeURIComponent(sourceId)}/test`, "POST"),
  setRadarSourceEnabled: (sourceId: string, enabled: boolean) => request<{ id: string; built_in: boolean; enabled: boolean }>(`/radar/sources/${encodeURIComponent(sourceId)}/enabled`, "PUT", { enabled }),
  deleteRadarSource: (sourceId: string) => request<{ id: string; deleted: boolean }>(`/radar/sources/${encodeURIComponent(sourceId)}`, "DELETE"),
