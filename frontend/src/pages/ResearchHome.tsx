...
import { loadPageTagState, resolveTag } from "@/features/tags/preferences";
    ...
    <div>
      ...
      <div className="grid gap-5 lg:grid-cols-[1.15fr_0.85fr]">
        ...
        <div className="space-y-5">
          <GlassCard><div className="mb-3 flex items-center gap-2"><Bookmark className="h-4 w-4 text-primary" /><h2 className="font-semibold">我的关注标签</h2></div><div className="flex flex-wrap gap-2">{followed.map((id) => <span key={id} className="rounded-full border border-primary/30 bg-primary/10 px-2.5 py-1 text-xs text-primary">{resolveTag(id)?.name || id}</span>)}</div></GlassCard>
          ...
          <GlassCard><h2 className="font-semibold">最近查看的行业</h2><p className="mt-3 text-sm">{resolveTag(industryTags.activeId)?.name || "暂无记录"}</p><p className="mt-1 text-xs text-muted-foreground">记录来自行业研究页面的本地标签偏好</p></GlassCard>
        </div>
      </div>
    </div>
  );
