...
export function HoldingDrawer({ open, holding, identified, existingCodes, saving, onClose, onSave }: HoldingDrawerProps) {
  ...
  const tagNames = useMemo(() => draft.tagIds.map((id) => id.replace(/^custom:/, "")), [draft.tagIds]);
  if (!open) return null;
  ...
  const submit = async () => {
    ...
    try {
      await onSave({
        ...
        verification_status: holding?.verification_status || "verified",
        manual_name: holding?.manual_name ?? null,
        replace: editing,
      });
    } catch (reason) {
