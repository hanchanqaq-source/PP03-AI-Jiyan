...
const emptyAnalysis: FundPortfolioAnalysisData = {
  overview: {
    ...
    intraday_estimate_complete: false, intraday_covered_count: 0, intraday_total_count: 0,
...
const populatedAnalysis: FundPortfolioAnalysisData = {
  ...
  overview: {
    ...
    intraday_estimate_complete: false, intraday_covered_count: 1, intraday_total_count: 2,
    pnl_complete: true,
  },
  holdings: [
    {
...
describe("PortfolioAnalysis schema v3 redesign", () => {
  ...
  it("renders a storage-only list with edit and delete controls when public analysis fails", async () => {
    vi.spyOn(api, "fundPortfolio").mockResolvedValue(savedPortfolio);
    vi.spyOn(api, "fundPortfolioAnalysis").mockRejectedValue(new Error("public providers offline"));

    render(<PortfolioAnalysis />);

    expect(await screen.findByText("组合分析暂不可用；本地持仓仍可独立读取和编辑。")).toBeInTheDocument();
    const rows = screen.getAllByTestId("holding-row");
    expect(rows).toHaveLength(2);
    expect(screen.getByText("基金 000001")).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "编辑持仓 基金 000001" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "删除持仓 基金 000001" })).toBeInTheDocument();
    expect(screen.getAllByText("数据待补充").length).toBeGreaterThan(0);
  });

  it("keeps a failed save in the drawer and shows the backend detail there", async () => {
    const user = userEvent.setup();
    vi.spyOn(api, "fundPortfolio").mockResolvedValue(savedPortfolio);
    vi.spyOn(api, "fundPortfolioAnalysis").mockResolvedValue(populatedAnalysis);
    vi.spyOn(api, "upsertFundHolding").mockRejectedValue(new Error("基金持仓备份失败：backup denied"));
    render(<PortfolioAnalysis />);
    await screen.findByText("华夏成长混合");
    ...
    const dialog = screen.getByRole("dialog", { name: "编辑持仓" });
    expect(await within(dialog).findByText("基金持仓备份失败：backup denied")).toBeInTheDocument();
    expect(dialog).toBeInTheDocument();
  });

  it("keeps a failed delete modal open and shows the backend detail inside it", async () => {
    const user = userEvent.setup();
    vi.spyOn(api, "fundPortfolio").mockResolvedValue(savedPortfolio);
    vi.spyOn(api, "fundPortfolioAnalysis").mockResolvedValue(populatedAnalysis);
    vi.spyOn(api, "deleteFundHolding").mockRejectedValue(new Error("基金持仓原子写入失败；原文件保持不变：denied"));
    render(<PortfolioAnalysis />);
    await screen.findByText("华夏成长混合");
    ...
    const dialog = screen.getByRole("dialog", { name: "确认删除持仓" });
    expect(await within(dialog).findByText("基金持仓原子写入失败；原文件保持不变：denied")).toBeInTheDocument();
    expect(dialog).toBeInTheDocument();
  });

  it("does not render a partial intraday sum as the complete headline", async () => {
    vi.spyOn(api, "fundPortfolio").mockResolvedValue(savedPortfolio);
    vi.spyOn(api, "fundPortfolioAnalysis").mockResolvedValue(populatedAnalysis);
    render(<PortfolioAnalysis />);

    await screen.findByText("华夏成长混合");
    const metric = screen.getByText("今日估算盈亏").closest("div");
    expect(metric).toHaveTextContent("覆盖 1/2");
    expect(metric).not.toHaveTextContent("10.56");
  });
