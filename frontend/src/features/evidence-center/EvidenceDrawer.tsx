...
function EvidenceList({ title, rows, empty }: { title: string; rows: EvidenceItem[]; empty: string }) {
  return <section><h3 className="font-semibold">{title}</h3>{rows.length ? <ul className="mt-2 space-y-2">{rows.map((row) => <li key={row.evidence_id} className="rounded-lg border border-border/50 bg-background/45 p-3 text-xs"><div className="flex flex-wrap items-center justify-between gap-2"><strong>{row.title || row.content_source}</strong><a href={row.canonical_url} target="_blank" rel="noreferrer" aria-label={`打开公开证据 ${row.title || row.content_source}`} className="inline-flex items-center gap-1 text-primary">打开公开证据<ExternalLink className="h-3 w-3" /></a></div><p className="mt-1 text-muted-foreground">内容来源：{row.content_source} · 采集来源：{row.collector_source}</p><p className="mt-1 text-muted-foreground">origin cluster：{row.origin_cluster}</p><p className="mt-2 leading-5 text-muted-foreground">{row.excerpt || "未提供公开摘录"}</p></li>)}</ul> : <p className="mt-2 text-xs text-muted-foreground">{empty}</p>}</section>;
}
  ...
  return <div className="fixed inset-0 z-50 bg-black/65" onMouseDown={(mouseEvent) => { if (mouseEvent.currentTarget === mouseEvent.target) onClose(); }}>
    <aside ref={dialogRef} role="dialog" aria-modal="true" aria-label="证据详情" className="ml-auto flex h-full w-full max-w-2xl flex-col border-l border-primary/25 bg-background/95 shadow-2xl backdrop-blur-xl">
      ...
      {loading || !event ? <div className="p-5 text-sm text-muted-foreground">正在读取真实证据链…</div> : <div className="flex-1 space-y-5 overflow-y-auto p-5 text-sm">
        ...
        <EvidenceList title="独立来源链" rows={event.independent_evidence} empty="暂无满足独立 origin cluster 的证据。" />
