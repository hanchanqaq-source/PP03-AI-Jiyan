...
const row = {
  event_id: "0123456789abcdef0123", title: "星河科技算力中心公告", published_at: "2026-08-18T07:00:00+00:00", verified_at: "2026-08-18T08:30:00+00:00", evidence_as_of: "2026-08-18T08:30:00+00:00", category: "company", related_tags: [{ id: "semiconductor", name: "半导体" }], core_claim: "公司公告建设算力中心", verification_status: "verified", verification_reason: "已有明确官方证据", verified_key_fields: [], pending_key_field_count: 1, conflicting_key_field_count: 0, primary_evidence_count: 1, independent_evidence_count: 1, syndicated_copy_count: 2, contradicting_evidence_count: 0, holding_relevance: "none", status_change_count: 1, latest_transition: transition,
};
const detail = {
  ...row, summary: "官方公开摘要", key_fields: [{ field_name: "money", raw_value: "12亿元", normalized_value: "CNY:1200000000", verification_status: "verified", evidence_ids: ["official-1"], reason: "官方字段一致" }],
...
const summary = { loaded: true, snapshot_id: "a".repeat(20), generated_at: "2026-08-18T08:30:00+00:00", counts: { verified: 3, corroborated: 3, unverified: 1, conflicting: 1, corrected: 0, disproved: 0 }, field_counts: { verified: 2, corroborated: 0, unverified: 1, conflicting: 0 }, admitted_count: 6, isolated_count: 2, last_refresh: { status: "completed", attempted_at: "2026-08-18T08:30:00+00:00" } };
...
describe("EvidenceCenter real verification workspace", () => {
  ...
  it("renders real summary and event rows without W0 fixture claims", async () => {
    ...
    expect(screen.getByText("星河科技算力中心公告")).toBeInTheDocument();
  ...
  it("requests real status filters and supports sorting", async () => {
    const user = userEvent.setup(); render(<EvidenceCenter />); await screen.findByText("星河科技算力中心公告");
  ...
  it("auto-opens query detail and runs a real refresh", async () => {
    ...
    expect(await screen.findByRole("dialog", { name: "证据详情" })).toHaveTextContent("星河科技算力中心公告");
  ...
  it("derives correction rows from real transition data", async () => {
    const user = userEvent.setup(); render(<EvidenceCenter />); await screen.findByText("星河科技算力中心公告"); await user.click(screen.getByRole("tab", { name: "更正记录" }));
    ...
    await user.click(screen.getByRole("button", { name: "查看记录 星河科技算力中心公告" }));
