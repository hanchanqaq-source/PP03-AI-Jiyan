...
export function SummarySection({ report }: { report: DisplayedIndustryReport }) {
  ...
  const trustedMetrics = new Map([...report.cycle, ...report.metrics, ...report.capital]
    .filter((metric) => metric.currentValue !== null
      && (metric.verificationStatus === "verified" || metric.verificationStatus === "corroborated"))
  ...
  return (
    <ReportSection id="overview" index="01" title="行业总览" eyebrow="Executive view">
      ...
      <div className="mt-7 grid gap-6 lg:grid-cols-[1.35fr_1fr]">
        <div><h3 className="text-sm font-semibold">判断依据</h3><ol className="mt-3 space-y-3 text-sm text-muted-foreground">{overview.basisMetricIds.length ? overview.basisMetricIds.map((item, index) => {
          const metric = trustedMetrics.get(item);
          return <li key={item} className="flex gap-3"><span className="font-mono text-primary">{index + 1}.</span><div><p className="font-mono text-foreground">{item}</p>{metric
            ? <div className="mt-2"><EvidenceDrawer metric={metric} triggerLabel={`查看 ${metric.label}总览证据`} /></div>
            : <p className="mt-1 text-xs leading-5">暂无可靠数据：未关联可信指标证据</p>}</div></li>;
        }) : <li>暂无可靠数据</li>}</ol></div>
