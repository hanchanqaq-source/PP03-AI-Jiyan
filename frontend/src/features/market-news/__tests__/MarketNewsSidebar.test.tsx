...
describe("MarketNewsSidebar", () => {
  it("shows the selected seven-day focus and holding impact range", () => {
    ...
    expect(screen.getByText("涉及持仓行业：0")).toBeInTheDocument();
    expect(screen.getByText("只涉及关注标签：0")).toBeInTheDocument();
    expect(screen.getByText(/东方人工智能主题混合C.*1 个事件/)).toBeInTheDocument();
  ...
  it("uses today wording only for a one-day selected range", () => {
    ...
    expect(screen.getByText("今天有 1 个事件与你的持仓相关")).toBeInTheDocument();
  });
});
