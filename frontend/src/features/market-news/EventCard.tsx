import { Building2, Clock3, ExternalLink, Layers3, Link2, Newspaper } from "lucide-react";
...
export function EventCard({ event, onOpenDetails }: { event: MarketNewsEvent; onOpenDetails: (event: MarketNewsEvent) => void }) {
  const relation = RELATION[event.relation_level];
  const originalUrl = event.original_links[0] || event.sources[0]?.original_url || "";
  return (
    <article className="group relative border-b border-border/55 py-5 pl-7 pr-1 last:border-b-0" aria-label={`${event.title}事件卡`}>
      ...
      <h3 className="mt-2.5 text-[17px] font-semibold leading-6 tracking-tight transition-colors group-hover:text-primary">{event.title}</h3>
      <p className={cn("mt-2 text-sm leading-6", event.summary_status === "ai_unavailable" ? "text-warning" : "text-muted-foreground")}>{event.summary}</p>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2 text-xs">
        ...
        <div className="ml-auto flex items-center gap-2">
          <button onClick={() => onOpenDetails(event)} aria-label={`查看事件详情 ${event.title}`} className="rounded-lg border border-border px-3 py-1.5 font-medium text-foreground hover:border-primary/45 hover:text-primary">查看事件详情</button>
          {originalUrl && <a href={originalUrl} target="_blank" rel="noreferrer" aria-label={`打开原始来源 ${event.title}`} className="inline-flex items-center gap-1 rounded-lg bg-primary px-3 py-1.5 font-semibold text-primary-foreground hover:bg-primary/90">打开原始来源<ExternalLink className="h-3 w-3" /></a>}
        </div>
      </div>
    </article>
  );
