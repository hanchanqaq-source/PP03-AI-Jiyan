...
class FundDataService:
    ...
    def _fetch(self, capability: str, *, cache_key: str, force_refresh: bool = False, **kwargs: Any) -> dict[str, Any]:
        ...
        preserved = self.cache.get(key, allow_stale=True)
        errors: list[str] = []
        ...
        if preserved:
            return self._section_from_cache(preserved, stale=True)
    ...
    def _derived_meta(
        ...
        *,
        ...
        metas = [section.get("meta") or {} for section in sections]

        def chain(field: str) -> str:
            values = [str(meta.get(field) or "").strip() for meta in metas]
            return "；".join(dict.fromkeys(value for value in values if value))

        stale = any(bool(meta.get("is_stale")) or meta.get("status") == "stale" for meta in metas)
        cached = any(bool(meta.get("is_cached")) for meta in metas)
        fetched = [str(meta.get("fetched_at")) for meta in metas if meta.get("fetched_at")]
        input_messages = [
            str(meta.get("message") or "").strip()
            for meta in metas
            if meta.get("status") in {"error", "unavailable", "stale"} and meta.get("message")
        ]
        messages = list(dict.fromkeys(part for part in [message, *input_messages] if part))
        final_status = "stale" if stale and status not in {"error", "unavailable"} else status
        return DataMeta(
            source_name=chain("source_name"),
            source_reference=chain("source_reference"),
            data_type=data_type,
            as_of_date=as_of_date,
            fetched_at=min(fetched) if fetched else self._now().isoformat(),
            status=final_status,
            is_cached=cached,
            is_stale=stale,
            provider=chain("provider"),
            fallback_used=any(bool(meta.get("fallback_used")) for meta in metas),
            message="；".join(messages),
            original_status=status if final_status == "stale" else None,
        ).to_dict()
    ...
    def _industry_exposure(
        ...
        holdings_as_of_date = holdings.get("as_of_date") or holdings.get("disclosure_date")
        ...
        for holding in holdings.get("holdings") or []:
            ...
            holding_industry_evidence.append({
                ...
                "holding_as_of_date": holdings_as_of_date,
                "holding_disclosure_date": holdings.get("public_date"),
            })
            for layer, field in (
        ...
        lookthrough = {
            ...
            "as_of_date": holdings_as_of_date,
            "disclosure_date": holdings.get("public_date"),
    ...
    def get_fund_analysis(self, code: str, force_refresh: bool = False) -> dict[str, Any]:
        ...
        if holdings.get("data"):
            ...
            exposure_meta = self._derived_meta(
                [profile, holdings, classifications, allocation],
                data_type="calculated_industry_exposure",
                as_of_date=(
                    holdings["data"].get("as_of_date")
                    or holdings["meta"].get("as_of_date")
                    or holdings["data"].get("disclosure_date")
                ),
            ...
            industry_exposure = {"data": exposure_data, "meta": exposure_meta}
        else:
        ...
        if holdings.get("data") and latest_data and snapshots.get("data"):
            ...
            estimate_data = calculate_intraday_estimate(
                ...
                holdings_as_of_date=date.fromisoformat(holdings_as_of) if holdings_as_of else None,
                estimate_time=self._now(),
            )
            estimate_status = "estimated" if estimate_data.get("status") == "estimated" else "unavailable"
            estimate_meta = self._derived_meta(
                [profile, latest_nav, holdings, snapshots],
                data_type="intraday_estimate",
                as_of_date=latest_data.get("nav_date"),
                status=estimate_status,
                message=estimate_data.get("message") or "",
            )
            intraday = {"data": estimate_data, "meta": estimate_meta}
        else:
            ...
            intraday = {
                ...
                "meta": self._derived_meta(
                    [profile, latest_nav, holdings, snapshots],
                    data_type="intraday_estimate",
                    as_of_date=latest_data.get("nav_date") if latest_data else None,
                    status="unavailable",
                    message=message,
                ),
            }
    ...
    def get_portfolio_analysis(self, holdings: list[dict[str, Any]], force_refresh: bool = False) -> dict[str, Any]:
        ...
        for holding in holdings:
            ...
            holdings_as_of_date = disclosed.get("as_of_date") or disclosed.get("disclosure_date")
            if holdings_as_of_date:
                try:
                    if (self._now().date() - date.fromisoformat(holdings_as_of_date)).days > 100:
                        risk_flags.append(f"{name} 的公开持仓报告期已超过一个季度")
                except ValueError:
                    pass
            enriched.append({
                "code": code,
        ...
        intraday_total_count = len(enriched)
        intraday_complete = bool(enriched) and estimable_count == intraday_total_count
        intraday_change = None
        intraday_message = "盘中估算暂不可用：当前基金类型或公开持仓不足以形成可靠估算"
        if intraday_complete and len(intraday_inputs) == intraday_total_count:
            ...
            intraday_message = f"今日估算仅覆盖 {estimable_count}/{intraday_total_count} 只基金；不展示不完整合计"
        return {
            "overview": {
                ...
                "intraday_estimated_profit_loss": round(intraday_estimated_profit_loss, 4) if intraday_complete else None,
                ...
                "intraday_estimate_complete": intraday_complete,
                "intraday_covered_count": estimable_count,
                "intraday_total_count": intraday_total_count,
