...
class AkshareEastmoneyProvider(BaseFundProvider):
    ...
    def _holdings(self, code: str) -> ProviderResult:
        latest_quarter: tuple[int, int] | None = None
        holdings: list[dict[str, Any]] = []
        for year in range(self._now().year, self._now().year - 3, -1):
            try:
                frame = self.ak.fund_portfolio_hold_em(symbol=code, date=str(year))
            except Exception:
                continue
            if frame is None or frame.empty or not {"季度", "序号"}.issubset(frame.columns):
                continue
            valid = [quarter for quarter in (self._quarter(value) for value in frame["季度"].tolist()) if quarter]
            if not valid:
                continue
            candidate_quarter = max(valid)
            latest_label = f"{candidate_quarter[0]}年{candidate_quarter[1]}季度"
            ...
            for _, row in selected.iterrows():
                ...
                if re.fullmatch(r"\d{6}", stock_code) is None or weight is None or weight <= 0:
                    continue
                candidate_holdings.append({
                    "stock_code": stock_code,
                    "stock_name": row.get("股票名称") or stock_code,
                    "weight_pct": weight,
                    "shares_10k": _number(row.get("持股数")),
                    "market_value_10k": _number(row.get("持仓市值")),
                })
            if candidate_holdings:
                latest_quarter = candidate_quarter
                holdings = candidate_holdings
                break
        if latest_quarter is None:
            raise ProviderUnavailable("东方财富未返回具有可识别报告期的公开股票持仓")
        month_day = {1: (3, 31), 2: (6, 30), 3: (9, 30), 4: (12, 31)}[latest_quarter[1]]
        as_of_date = date(latest_quarter[0], *month_day).isoformat()
        return ProviderResult(
            data={
            ...
            data_type="disclosed_holdings", as_of_date=as_of_date, status="disclosed",
            message="基金持仓来自定期报告；日期为报告期截至日，不是公开发布日期；公开日期字段暂缺",
        )
    ...
    def _industry_allocation(self, code: str) -> ProviderResult:
        ...
        for year in range(self._now().year, self._now().year - 3, -1):
            try:
                frame = self.ak.fund_portfolio_industry_allocation_em(symbol=code, date=str(year))
            except Exception:
                continue
            if frame is None or frame.empty or "截止时间" not in frame.columns:
                continue
            dates = []
            for value in frame["截止时间"].tolist():
            ...
            if not dates:
                continue
            candidate_latest = max(dates)
            selected = frame[frame["截止时间"].astype(str).str.slice(0, 10) == candidate_latest]
            ...
            if candidate_industries:
                ...
                break
        if latest is None:
            raise ProviderUnavailable("东方财富未返回具有可识别报告期的公开行业配置")
        return ProviderResult(
