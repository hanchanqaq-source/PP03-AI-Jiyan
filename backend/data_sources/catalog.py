...
def _capabilities() -> tuple[CapabilityDescriptor, ...]:
    return (
        ...
        CapabilityDescriptor("stock_snapshot", "股票行情", "quote", 3 * _DAY_SECONDS, True, "market_quote", "intraday", ("tencent",), ("eastmoney", "finnhub"), ()),
        ...
        CapabilityDescriptor("stock_history", "股票历史行情", "quote", 3 * _DAY_SECONDS, True, "CNY", "daily", ("baostock",), ("alpha_vantage", "twelve_data"), ()),
        ...
        CapabilityDescriptor("macro_series", "宏观时间序列", "macro", 31 * _DAY_SECONDS, True, "provider_native", "provider_native", ("oecd",), ("imf", "fred", "eia", "nasdaq_data_link"), ()),
        CapabilityDescriptor("fund_holdings", "基金持仓明细", "fund", 200 * _DAY_SECONDS, True, "provider_native", "quarterly", (), ("tushare",), ()),
        CapabilityDescriptor("news_discovery", "资讯候选发现", "news", None, False, "candidate", "event_driven", (), ("gdelt", "finnhub", "news_api"), ()),
        CapabilityDescriptor("industry_price_snapshot", "行业公开价格当前快照候选", "industry", _DAY_SECONDS, False, "USD", "current_snapshot", (), ("trendforce_public_price",), ()),
    )
...
def _families() -> tuple[SourceFamily, ...]:
    return (
        ...
        SourceFamily("alpha_vantage", "Alpha Vantage", "global", "overseas", (SourceRole.FALLBACK_DATA, SourceRole.MARKET_DATA, SourceRole.CROSS_CHECK), True, "alpha_vantage_account_terms_apply", CatalogStatus.UNCONFIGURED),
        SourceFamily("finnhub", "Finnhub", "global", "overseas", (SourceRole.MARKET_DATA, SourceRole.FALLBACK_DATA, SourceRole.COLLECTOR, SourceRole.CANDIDATE), False, "finnhub_account_terms_apply", CatalogStatus.UNCONFIGURED),
        ...
        SourceFamily("trendforce_public_price", "TrendForce public current price snapshot candidate", "global", "semiconductor", (SourceRole.CANDIDATE,), False, "public_page_reuse_license_unverified", CatalogStatus.UNCONFIGURED),
    )
...
def _static_adapters() -> tuple[AdapterDescriptor, ...]:
    return (
        ...
        _adapter("alpha-vantage", "Alpha Vantage", "alpha_vantage", "http_client", (SourceRole.FALLBACK_DATA, SourceRole.MARKET_DATA, SourceRole.CROSS_CHECK), ("stock_history",), "https://www.alphavantage.co/", 120, default_enabled=False, catalog_status=CatalogStatus.UNCONFIGURED, license_note="Alpha Vantage account terms apply; capability and quota depend on the actual plan.", usage_note="Low-frequency fallback only; never promoted to primary market data.", billing_model=BillingModel.FREEMIUM, auth_type="api_key", credential_env_names=("ALPHA_VANTAGE_API_KEY",), data_delay="以 Alpha Vantage 实际返回为准", quota_policy="以账户实际套餐和配额为准", cost_policy="套餐成本未知；无可信能力级权益时禁止请求"),
        _adapter("finnhub", "Finnhub", "finnhub", "http_client", (SourceRole.MARKET_DATA, SourceRole.FALLBACK_DATA, SourceRole.COLLECTOR, SourceRole.CANDIDATE), ("stock_snapshot", "news_discovery"), "https://finnhub.io/", 130, default_enabled=False, catalog_status=CatalogStatus.UNCONFIGURED, license_note="Finnhub account terms apply; plan and quota must be checked per capability.", usage_note="Market reference and news discovery only; collected news never replaces SEC or official evidence.", billing_model=BillingModel.FREEMIUM, auth_type="api_key", credential_env_names=("FINNHUB_API_KEY",), data_delay="以 Finnhub 和原发布者实际时间为准", quota_policy="以账户实际套餐和配额为准", cost_policy="套餐成本未知；无可信能力级权益时禁止请求"),
