...
def test_akshare_provider_uses_latest_disclosed_quarter_and_scale_date():
    ...
    assert holdings.data["as_of_date"] == "2026-06-30"
    assert holdings.data["disclosure_date"] is None
    assert holdings.data["public_date"] is None
    assert holdings.as_of_date == "2026-06-30"
...
def test_akshare_provider_falls_back_to_recent_prior_year_disclosures():
    class PriorYearAkshare(FakeAkshare):
        holdings_years = []
        industry_years = []

        @classmethod
        def fund_portfolio_hold_em(cls, symbol, date):
            cls.holdings_years.append(date)
            if date == "2026":
                return pd.DataFrame(columns=["序号", "股票代码", "股票名称", "占净值比例", "持股数", "持仓市值", "季度"])
            return pd.DataFrame([
                [1, "600000", "浦发银行", 8.0, 10.0, 100.0, "2025年4季度股票投资明细"],
            ], columns=["序号", "股票代码", "股票名称", "占净值比例", "持股数", "持仓市值", "季度"])

        @classmethod
        def fund_portfolio_industry_allocation_em(cls, symbol, date):
            cls.industry_years.append(date)
            if date == "2026":
                return pd.DataFrame(columns=["序号", "行业类别", "占净值比例", "市值", "截止时间"])
            return pd.DataFrame([
                [1, "金融业", 60.0, 1200.0, "2025-12-31"],
            ], columns=["序号", "行业类别", "占净值比例", "市值", "截止时间"])

    provider = AkshareEastmoneyProvider(
        ak_module=PriorYearAkshare(),
        now=lambda: datetime(2026, 2, 10, tzinfo=timezone.utc),
    )

    holdings = provider.fetch("holdings", code="000001")
    industry = provider.fetch("industry_allocation", code="000001")

    assert PriorYearAkshare.holdings_years == ["2026", "2025"]
    assert PriorYearAkshare.industry_years == ["2026", "2025"]
    assert holdings.data["report_period"] == "2025-Q4"
    assert holdings.data["as_of_date"] == "2025-12-31"
    assert holdings.data["disclosure_date"] is None
    assert industry.data["as_of_date"] == "2025-12-31"
