...
### Windows 本地启动（PP03 原生收敛版）

PP03 直接扩展 Vibe-Research 原生模块，不再提供平行的“投研首页 / 市场资讯 / 行业研究 / 持仓分析 / 证据中心”。资讯来源管理位于 `/intel/sources`，产业小模块位于 `/signals`。推荐使用 PowerShell 分别启动后端和前端：
...
- 内置资讯源仍由仓库版本控制；新增、停用和健康状态原子写入 `%VR_DATA_DIR%\news-sources.custom.json`。
- 自定义 RSS/Atom 在保存前执行公网地址、重定向、响应大小、TLS、XML 结构以及 `title/link` 校验；内网/本机 URL 会被拒绝。
- API 来源只接受已经实现并登记的显式适配器，不会猜测任意 JSON 字段。
- 旧版基金、证据、来源体检和行业研究用户文件不删除，状态为 `PRESERVED_BUT_NOT_CURRENTLY_EXPOSED`；详见 `docs/migrations/2026-08-27-native-radar-convergence.md`。
