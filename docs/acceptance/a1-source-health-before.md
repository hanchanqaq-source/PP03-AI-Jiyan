...
- Task 7 页面验收前自动验证：后端 `357 passed`（其中 13 个标记为 `live` 的联网测试在本次运行通过，不能外推为离线或长期 PASS）、0 failed、0 skipped、2 warnings；前端 Vitest `23 files / 97 tests passed`；legacy `16 passed`；生产构建 exit 0。
